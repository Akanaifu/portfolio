...
function calculateTotals(data) {
	...
	const totalPrestees = data.reduce((sum, item) => sum + parseHours(item.heures_prestees), 0);
	const totalValorisees = data.reduce((sum, item) => sum + parseHours(item.heures_valorisees), 0);
...
async function createTab(idTab, file) {
	try {
		...
		if (window.updateVeloHours) {
			...
			if (veloStats) {
				const veloEntry = data.find((item) => item.nom.toLowerCase() === "vélo");
				if (veloEntry) {
		...
		tbody.innerHTML = buildGroupedTbodyHtml(groups, headers) + buildTotalRow(headers, totals);
	} catch (err) {
		console.error(err);
	}
}
