...
async function updateVeloHours() {
	try {
		...
		if (
			...
			console.info("ℹ️ Strava non configuré. Les heures de vélo affichées proviennent du JSON.");
			return null;
		}
		...
		const strava = new StravaIntegration(accessToken, window.STRAVA_CONFIG.athleteId);
		...
		if (error.message.includes("401")) {
			...
			console.error("2. Votre application Strava a les permissions 'activity:read_all'");
